...
class GRUVarLenSeq(nn.Module):
    ...
    def __init__(self, mode, embedding, num_rec_layers, num_directions, hidden_size, dropout):
        super().__init__()
        ...
        self.recurrent = nn.GRU(
            input_size=self.embeddings.shape[1], hidden_size=self.hidden_size, num_layers=self.num_rec_layers,
    ...
    def forward(self, x):
        ...
        if self.num_directions != 1 or self.num_rec_layers != 1:
            h = torch.transpose(h[::self.num_rec_layers], 0, 1).contiguous()
